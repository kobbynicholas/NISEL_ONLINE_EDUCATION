...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["classroom_action"])) {
    try {
        ...
        if ($action === "start_class") {
            ...
            $room = trim((string)($current["live_room_code"] ?? $room_code));
            if ($room === "") {
                $room = $room_code;
            }
...
.layout{height:calc(100vh - 68px);display:grid;grid-template-columns:minmax(0,1fr) 340px}.stage{position:relative;min-width:0;background:#020711;overflow:hidden}.remote-wrap{position:absolute;inset:0;background:radial-gradient(circle at 50% 40%,#0a35521c,transparent 38%),#020711}.remote-placeholder{position:absolute;inset:0;display:grid;place-items:center;text-align:center;color:#8ea1b6;padding:30px}.remote-placeholder div{max-width:460px}.remote-placeholder .icon{font-size:48px;margin-bottom:12px}.remote-placeholder h2{margin:0 0 8px;color:#d9e5ef}.remote-placeholder p{margin:0;line-height:1.6;font-size:13px}
...
<aside class="sidebar">
    <div class="lesson">
        ...
        <div class="rows">
            ...
            <div class="row"><span>Room</span><span><?= e($room_code) ?></span></div>
...
async function createOffer() {
    if (!peerConnection || !started || offerInProgress) return;
    offerInProgress = true;
    try {
        const offer = await peerConnection.createOffer();
        await peerConnection.setLocalDescription(offer);
        await sendSignal("offer", peerConnection.localDescription.toJSON ? peerConnection.localDescription.toJSON() : peerConnection.localDescription);
    } catch (e) {
        console.error("Offer error", e);
        toastMessage("Unable to create the video connection.", true);
    } finally {
